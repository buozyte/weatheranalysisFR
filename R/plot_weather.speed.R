plot_weather.speed <- function(x, yr = NULL, both = TRUE) {
  date <- as.Date(paste(row.names(klimaFR), "01"), "%b %Y %d")
  all <- data.frame("date" = date, "spee" = as.numeric(x))
  spe <- data.frame("date" = date[1:12], "spe50" = as.numeric(x)[1:12],
                    "spe80" = as.numeric(x)[361:372], "spe10" = as.numeric(x)[721:732])

  p1 <- ggplot(all, aes(x=date, y=spee)) +
    geom_line() +
    labs(y="Bft", x="year", title="Spe over time") +
  ...
  if (!is.null(yr)) {
    ...
    if (len == 1) {
      ...
      for (i in 1:len) {
        ...
        if (is.null(spen)) {
          spen <- data.frame("date" = datef, spee1 = as.numeric(x)[intvs])
          colnames(spen)[i+1] <- paste0("Y", yrsor[i])
        } else {
          spen <- data.frame(spen, speet = as.numeric(x)[intvs])
          colnames(spen)[i+1] <- paste0("Y", yrsor[i])
        }
